...
@for %%i in ("%SavesDir%\%SteamId%\%File1%.%FileTypeExt%") do (
	@for /f "tokens=1-5 delims=.: " %%j in ("%%~ti") do (
		@rem @echo %%l%%k%%j%%m%%n
...
@for %%i in ("%SavesDir%\%SteamId%\%File1%.%FileTypeExt%") do (@for /f "tokens=1-5 delims=.: " %%j in ("%%~ti") do @echo %%l%%k%%j%%m%%n>%temp%\tmpPrevFileDateTimeStamp.txt)
