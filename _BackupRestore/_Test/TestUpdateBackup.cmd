		...
		if %isStep% LEQ 2 (
			for /f "delims=|" %%L in (!7z_List[%isStep%]!) Do (
				if exist "!Dir[%isStep%]!\%%L" (
					ROBOCOPY "%%L" "%BackupsDir%\Users\%USERNAME%\AppData\!FileN[%isStep%]!\%%L" %RCParams% /XF !RC_ExcludeRecurseFileList! /XD !RC_ExcludeDirList[%isStep%]! %RCFileDataFlags%
				)
			)
		)
		if %isStep% GEQ 4 (
			if %isStep% LEQ 6 (
				for /f "delims=|" %%L in (!7z_List[%isStep%]!) Do (
					if exist "!Dir[%isStep%]!\%%L" (
						ROBOCOPY "%%L" "%BackupsDir%\!FileN[%isStep%]!\%%L" %RCParams% /XF !RC_ExcludeRecurseFileList! /XD !RC_ExcludeDirList[%isStep%]! %RCFileDataFlags%
					)
				)
			)
		)
