...
@set Aria2Dir=%ScriptsRootPath%\_Tools\[]Aria2
...
@rem %WgetDir%\wget.exe -t 0 --retry-connrefused -c --no-check-certificate --auth-no-challenge --http-user Anonimous --user Anonimous -O "%DownloadsDir%\%InputFileName%" --user-agent="Mozilla/5.0 (Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/55.0.2883.75 Safari/537.36" "%Input%"

%Aria2Dir%\aria2c.exe "%Input%" -d "%DownloadsDir%" -o "%InputFileName%" --file-allocation prealloc -c true
