...
@rem 
@echo "Previous entry - FileDateTimeStamp: %FileDateTimeStamp%"
...
@for %%i in ("%SavesDir%\%SteamId%\%File1%.%FileType%") do (
	@for /f "tokens=1-5 delims=.: " %%j in ("%%~ti") do (
		...
		@if "%%l%%k%%j%%m%%n" NEQ "%FileDateTimeStamp%" (
			@if "%%l%%k%%j%%m%%n" lss "%DateTimeStamp%" (
				@rem 
				@echo [92m " The date and time of the file is not equal to the previous entry. " [0m 
				@rem 
				@echo [92m " %%l%%k%%j%%m%%n " [0m 
				@rem 
				@echo [92m " FileDateTimeStamp: %FileDateTimeStamp% " [0m 
				@rem @echo "The date and time of the file is not equal to the previous entry."
				@rem @echo "FileDateTimeStamp: %FileDateTimeStamp%"
				@rem !7za! u -r0 !7zaKeyString2! "%AutoSavesDir%\%AutoSaveFileName%.7z" "%SavesDir%\*" -w"%Temp%"
			)
		)
	)
)
@for %%i in ("%SavesDir%\%SteamId%\%File1%.%FileType%") do (@for /f "tokens=1-5 delims=.: " %%j in ("%%~ti") do @set "FileDateTimeStamp=%%l%%k%%j%%m%%n")
@rem 
