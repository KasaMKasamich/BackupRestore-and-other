...
rem  int
rem  RoboCopy
set RCParams=/S /B /R:3 /W:2 /SL /MT:8 /XA:sh /XO
set RCParams2=/S /mir /B /R:3 /W:2 /SL /MT:8 /XO
...
rem >> ===================================
	...
	if %isStep% LEQ 2 (
		if not %isStep% == 0 (
			ROBOCOPY "!Dir[%isStep%]!" "!TempRoboCopDir!\%isStep%" %RCParams% /LOG:"!RCLogFile!" /XF !RC_ExcludeRecurseFileList! /XD !RC_ExcludeDirList[%isStep%]! %RCFileDataFlags%
			REM for /f "delims=|" %%L in (!7z_List[%isStep%]!) Do (
				REM if exist "!Dir[%isStep%]!\%%L" (
					REM ROBOCOPY "%%L" "!TempRoboCopDir!\%isStep%\%%L" %RCParams% /LOG:"!RCLogFile!" /XF !RC_ExcludeRecurseFileList! /XD !RC_ExcludeDirList[%isStep%]! %RCFileDataFlags%
				REM )
			REM )
		)
	)
