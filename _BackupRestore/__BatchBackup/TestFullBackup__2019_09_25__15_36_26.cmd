...
:Test
@rem set isStep=3
if %isStep% == 0 (set /a isStep+=!isStep!+1)
echo %isStep%
rem 
exit /b
echo ======================================================
...
:Switch
@timeout /t 2 >nul
if not exist "!TempAppData[%isStep%]!"\%FileN[0]%.7z" if %isStep% == 0 (set /a isStep=%isStep%+1)

if not exist "!TempAppData[0]!\LocalLow.7z" if %isStep% == 1 (set isStep=2)
