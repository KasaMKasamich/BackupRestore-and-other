...
rem раз два три команда rd удаляет только пустые папки (папки с файлами и файлы не трогает), если ее использовать без ключа /s
...
:: if errorlevel 8 echo [95mNot_enough_memory[0m rem goto:not_enough_memory
:: if errorlevel 7 echo [94mCommand_line_error[0m rem goto:command_line_error
:: if errorlevel 2 echo [91mFatal_Error[0m rem goto:fatal_error
:: if errorlevel 1 echo [93mOk_Warnings[0m rem goto:ok_warnings
:: 
pause
