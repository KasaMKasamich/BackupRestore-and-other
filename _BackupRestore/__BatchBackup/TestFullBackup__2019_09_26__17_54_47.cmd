...
set isCLS=1
if %DM% == 1 (
	if %cleantemp% GEQ 1 ( powershell -Command "Get-ChildItem -Path "%TempSourceBackupDir%\*" -Directory  | foreach { $_.Delete()}" )
	if %cleantemp% == 2 ( powershell -Command "Get-ChildItem -Path "%TempRoboCopDir[0]%\*" -Directory  | foreach { $_.Delete()}" )
	if %cleantemp% == 3 ( powershell -Command "Get-ChildItem -Path "%TempBackupDir%\Users\*" -Directory  | foreach { $_.Delete()}" )
	set isStep=1
	@timeout /t 3 >nul
)
