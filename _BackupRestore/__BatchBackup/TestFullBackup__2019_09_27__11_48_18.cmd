...
Echo WScript.Echo Round(CreateObject("Scripting.FileSystemObject").GetFolder(WScript.Arguments(0)).Size/2^^20,7)>GetDirSize.vbs
for /f "usebackq delims=;" %%A in (`dir /b *.*`) do (
	For /F "delims=" %%B In ('CScript //Nologo "%TempSourceBackupDir%"\GetDirSize.vbs "%%A"') Do (
		...
		if %%B geq "0,00001" Echo Size of %%A is !Bytes! MB. >> %LogFile%
	)
)
echo [92m " =================================== Sandbox =================================== " [0m 
set DM=1
