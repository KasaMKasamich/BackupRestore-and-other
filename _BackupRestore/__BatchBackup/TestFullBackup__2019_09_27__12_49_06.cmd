...
Echo WScript.Echo Round(CreateObject("Scripting.FileSystemObject").GetFolder(WScript.Arguments(0)).Size+0,000008/2^^20,6)>GetDirSize.vbs
for /f "usebackq delims=;" %%A in (`dir /b *.*`) do (
	For /F "delims=" %%B In ('CScript //Nologo "%TempSourceBackupDir%"\GetDirSize.vbs "%%A"') Do (
		Set Bytes=%%B
		rem set /A iBytes=(%%B+0,000008)
