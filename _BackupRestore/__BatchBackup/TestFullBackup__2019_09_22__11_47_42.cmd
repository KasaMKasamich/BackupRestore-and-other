...
rem !7za! u !7zaKeyString! "%TempBackupDir%\Local.7z" "%TempSourceBackupDir%\*" -w"%Temp%" -x@"%ListLocalExclude%" -xr@"%ListRecurseExclude%"
...
For /D %%A In ("%TempSourceBackupDir%\*") Do (
	For /F "delims=" %%B In ('CScript //Nologo %TempSourceBackupDir%\GetDirSize.vbs "%%A"') Do (
		if %%B geq !MB! !7za! u -y -r- !7zaKeyString! `-x!.tmp` -x@!ListLocalExclude! -xr@!ListRecurseExclude! "!TempBackupDir!\%%~nxA.7z" "%%A"
	)
)
