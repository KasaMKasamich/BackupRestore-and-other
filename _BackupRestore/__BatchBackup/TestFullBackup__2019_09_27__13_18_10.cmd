...
Echo WScript.Echo Round(CreateObject("Scripting.FileSystemObject").GetFolder(WScript.Arguments(0)).Size/1^^20,6)>GetDirSize.vbs
for /f "usebackq delims=;" %%A in (`dir /b *.*`) do (
	For /F "delims=" %%B In ('CScript //Nologo "%TempSourceBackupDir%"\GetDirSize.vbs "%%A"') Do (
		Set Bytes=%%B
		rem set /A "fBytes=!Bytes!+0.000008"
		Set /A IntMB=!Bytes!/1048576
		Set /A IntKB=!Bytes!/1024
		Set /A FloatMB=!Bytes!%%1048576/10000
		Set /A FloatKB=!Bytes!%%1024/10
		if %%B geq "0.000001" Echo Size of %%A is !Bytes:~0,-3!.!Bytes:~0,-3!.!Bytes:~-6! MB. >> %LogFile%
	)
)
echo [92m " =================================== Sandbox =================================== " [0m 
set DM=1
