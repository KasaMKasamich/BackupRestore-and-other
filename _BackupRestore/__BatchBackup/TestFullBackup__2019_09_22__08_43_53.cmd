...
@timeout /t 5 >nul
for /f "usebackq delims=" %%A in (`dir /a:d /s /b "%TempSourceBackupDir%\*Crash*"`) do ( rmdir /q "%TempSourceBackupDir%\%%~nxA" )
for /f "usebackq delims=" %%A in (`dir /a:d /s /b "%TempSourceBackupDir%\*Cache*"`) do ( rmdir /q "%TempSourceBackupDir%\%%~nxA" )
for /f "usebackq delims=" %%A in (`dir /a:d /s /b "%TempSourceBackupDir%\*Tile*"`) do ( rmdir /q "%TempSourceBackupDir%\%%~nxA" )
for /f "usebackq delims=" %%A in (`dir /a:d /s /b "%TempSourceBackupDir%\*Temp*"`) do ( rmdir /q "%TempSourceBackupDir%\%%~nxA" )
for /f "usebackq delims=" %%A in (`dir /a:d /s /b "%TempSourceBackupDir%\*font*"`) do ( rmdir /q "%TempSourceBackupDir%\%%~nxA" )
for /f "usebackq delims=" %%A in (`dir /a:d /s /b "%TempSourceBackupDir%\{*}"`) do ( rmdir /q "%TempSourceBackupDir%\%%~nxA" )
...
For /D %%A In ("%TempSourceBackupDir%\*") Do (
	For /F "delims=" %%B In ('CScript //Nologo %TempSourceBackupDir%\GetDirSize.vbs "%%A"') Do (
		if %%B geq !MB! !7za! u !7zaKeyString! "%TempBackupDir%\%%~nxA.7z" "%%A" -w"%Temp%" -x!^^"IconCache*.*" -x!^^"thumbcache*.*" -x!^^"*.tmp" -x!^^"*.dmp" -x!^^"*.cookie" -x!^^"*.log" -x!^^"*.vbs" -x!^^"*.7z" -x@"!ListLocalExclude!" -xr@"!ListRecurseExclude!"
	)
)
@timeout /t 2 >nul
