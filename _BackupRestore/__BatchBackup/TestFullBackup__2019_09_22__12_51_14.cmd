...
For /D %%A In ("%TempSourceBackupDir%\*") Do (
	For /F "delims=" %%B In ('CScript //Nologo %TempSourceBackupDir%\GetDirSize.vbs "%%A"') Do (
		if %%B geq !MB! !7za! u -y -r !7zaKeyString! "!TempBackupDir!\%%~nxA.7z" "%%A" '-x!Comodo.folder' '-xr!*.tmp' '-xr!*.dmp' '-xr!*.cookie' '-xr!*.log' '-xr!*.vbs' '-xr!*.7z' '-xr!Logs' '-xr!Temp' '-xr!Crash' '-xr!IceDragon'
	)
)
