...
Echo WScript.Echo Round(CreateObject("Scripting.FileSystemObject").GetFolder(WScript.Arguments(0)).Size)>GetDirSize.vbs

rem for /f "usebackq delims=;" %%A in (`dir /b *.*`) do ( If %%~zA LSS 1 Echo Size of %%A is %%~zA MB. >> %LogFile% )
for /f "usebackq delims=;" %%A in (`dir /b *.*`) do (
	For /F "delims=" %%B In ('CScript //Nologo "%TempSourceBackupDir%"\GetDirSize.vbs "%%A"') Do (
		Set Bytes=%%B
		Echo Size of %%A is !Bytes! MB. >> %LogFile%
	)
)
echo [92m " =================================== Sandbox =================================== " [0m 
set DM=1
