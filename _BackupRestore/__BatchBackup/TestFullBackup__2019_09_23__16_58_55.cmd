...
rem for /f "delims=|" %%L in (!ListL_Exclude!) Do ( echo %%~nxL  --  %%L )
for /f "delims=|" %%L in (!ListL_Exclude!) Do (
	for /f "usebackq delims=" %%i in (`dir /a:d /s /b "%TempSourceBackupDir%\%%~nxL"`) Do (
		if exist "%TempSourceBackupDir%\%%~nxi" rmdir /q "%TempSourceBackupDir%\%%~nxi"
	)
)
@timeout /t 92 >nul
