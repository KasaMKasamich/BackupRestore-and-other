...
@timeout 2 >nul
start /b /REALTIME /wait F:\_BackupRestore\7z\1900\x64\7za.exe a -y -ssw "%TempBackupDir%\2.7z" "%TempSourceBackupDir%\*" -r0 -w"%TempBackupDir%" -i@"%ListLocalInclude%" -x@"%ListLocalExclude%" -xr@"%ListRecurseExclude%" -slp -scsWIN -mmt=on -mx9 -ms=off -bse1 >> %LogsDir%\%FileNameSec01%_Local_err_7z.log -bso1 >> %LogsDir%\%FileNameSec01%_Local_7z.log
...
For /D %%A In ("%TempSourceBackupDir%\*") Do (
	start /b /REALTIME /wait F:\_BackupRestore\7z\1900\x64\7za.exe a -y -ssw "%TempBackupDir%\%%~nxA.7z" "%%A" -r0 -w"%TempBackupDir%" -i@"%ListLocalInclude%" -x@"%ListLocalExclude%" -xr@"%ListRecurseExclude%" -bse1 -slp -scsWIN -mmt=on -mx9 -ms=off > %LogsDir%\%FileNameSec01%_%%~nxA_7z.log
)
@timeout 2 >nul
