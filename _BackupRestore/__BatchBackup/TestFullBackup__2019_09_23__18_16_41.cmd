...
for /f "delims=|" %%L in (!ListL_Exclude!) Do (
	for /d %%A In ("%Dir001%\*") Do (
		for /f "usebackq delims=" %%i in (`dir /a:d /-s /b "%Dir001%\%%L"`) Do (
			rem if not "%Dir001%\%%i"=="%Dir001%\%%~nxA" MKLINK /d /h /j "%%~nxA" "%%A"
			if not exist "%TempSourceBackupDir%\%%i" MKLINK /d /h /j "%%~nxA" "%%A"
		)
	)
)
@timeout /t 92 >nul
