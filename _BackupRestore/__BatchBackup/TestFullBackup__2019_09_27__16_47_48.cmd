...
for /f "usebackq delims=;" %%A in (`dir /b *.*`) do (
	For /F "delims=" %%B In ('CScript //Nologo GetDirSize.vbs "%%A"') Do (
		if %%B LSS 1 Echo Size of %%A is %%B MB. >> %LogFile%
		if %%B LSS 2 !7za! a -u -y -r -sdel !7zaKeyString! "C:\_BR\_Deleted\_Deleted.7z" "%%A" >> %LogFile%
...
for /f "usebackq delims=;" %%A in (`dir /s /b *.*`) do (
	For /F "delims=" %%B In ('CScript //Nologo "GetDirSize.vbs "%%A"') Do (
		if "%%~nxA" == "%%A" (
			if %%B LSS 1 del /f /s /q "%%A" >> %LogFile%
			if %%B LSS 1 Echo Size of %%A is %%B bytes. >> %LogFile%
		)
	)
)
for /f "usebackq delims=;" %%A in (`dir /s /b *.*`) do (
