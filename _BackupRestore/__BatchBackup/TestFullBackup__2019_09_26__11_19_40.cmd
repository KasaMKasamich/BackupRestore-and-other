...
@timeout /t 5 >nul
if %isStep% == 1 (
	powershell -Command "Get-ChildItem -Path "%TempSourceBackupDir%2\*" -Directory -Include %ListDir%\ExcludeLocal.txt  | foreach { $_.Delete()}"
)
if %isStep% == 2 (
	powershell -Command "Get-ChildItem -Path "%TempSourceBackupDir%2\*" -Directory  | foreach { $_.Delete()}"
)
@rem 
exit /b
echo ======================================================
