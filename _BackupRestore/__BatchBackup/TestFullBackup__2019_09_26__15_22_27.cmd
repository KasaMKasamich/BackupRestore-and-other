...
if %isStep% == 1 (
	@rem powershell -Command "Get-ChildItem -Path "%TempSourceBackupDir%2\*" -Directory -Include !List_Exclude[%isStep%]!  | foreach { $_.Delete()}"
)
if %isStep% == 2 (
	@rem powershell -Command "Get-ChildItem -Path "%TempSourceBackupDir%2\*" -Directory  | foreach { $_.Delete()}"
)
@rem 
exit /b
echo ======================================================
