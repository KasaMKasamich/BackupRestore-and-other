...
@timeout 2 >nul
start /b /REALTIME /wait F:\_BackupRestore\7z\1900\x64\7za.exe a -y -ssw "%TempBackupDir%\2.7z" "%TempSourceBackupDir%\*" -r0 -w"%TempBackupDir%" -i@"%ListLocalInclude%" -x@"%ListLocalExclude%" -xr@"%ListRecurseExclude%" -slp -scsWIN -mmt=on -mx9 -ms=off -bse2 >> %LogsDir%\%FileNameSec01%_Local_err_7z.log
...
For /D %%A In ("%TempSourceBackupDir%\*") Do (
	For /F "delims=" %%B In ('CScript //Nologo %TempSourceBackupDir%\GetDirSize.vbs "%%A"') Do (
		if %%B geq !MB! start /b /REALTIME /wait F:\_BackupRestore\7z\1900\x64\7za.exe a -y -ssw "%TempBackupDir%\%%~nxA.7z" "%%A" -r0 -w"%TempBackupDir%" -i@"%ListLocalInclude%" -x@"%ListLocalExclude%" -xr@"%ListRecurseExclude%" -bse2 -slp -scsWIN -mmt=on -mx9 -ms=off > %LogsDir%\%FileNameSec01%_%%~nxA_7z.log
	)
