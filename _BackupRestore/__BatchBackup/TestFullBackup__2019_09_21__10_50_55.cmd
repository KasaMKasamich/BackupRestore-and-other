...
start /b /wait F:\_BackupRestore\7za\7za.exe a -t7z -ssw -mx9 -slp -scsWIN -mmt=4 -ms=off "%TempBackupDir%\Yandex.7z" "%TempBackupDir%\Yandex" -r0 -x@"%ListLocalExclude%" -xr@"%ListRecurseExclude%"
...
For /D %%A In ("%TempSourceBackupDir%\*") Do (
	For /F "delims=" %%B In ('CScript //Nologo %TempSourceBackupDir%\GetDirSize.vbs "%%A"') Do (
		if %%B geq !MB! start /b /REALTIME /wait F:\_BackupRestore\7z\1900\x64\7za.exe a -t7z -ssw -mx9 -slp -scsWIN -mmt=4 -ms=off -y "%TempBackupDir%\%%~nxA.7z" "%%A" -r0 -w"%TempBackupDir%" -i@"%ListLocalInclude%" -x@"%ListLocalExclude%" -xr@"%ListRecurseExclude%"
	)
