...
@for %%i in ("%SavesDir%\%SteamId%\%File1%.%FileType%") do (
	@for /f "tokens=1-5 delims=.: " %%j in ("%%~ti") do (
		...
		@if "%%l%%k%%j%%m%%n" EQU "%PreviousFileDateTimeStamp%" (
			...
			@rem 
			@echo [92m " File Date Time Stamp: %%l%%k%%j%%m%%n " [0m 
			@rem 
			@echo [92m " Previous entry of File Date Time Stamp: %PreviousFileDateTimeStamp% " [0m 
		)
...
@rem 
@echo "Previous entry of File Date Time Stamp: %PreviousFileDateTimeStamp%"
