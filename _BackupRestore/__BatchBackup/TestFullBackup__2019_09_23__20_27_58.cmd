...
set TempBackupDir[0]=C:\_BR\Local
set TempBackupDir[1]=C:\_BR\LocalLow
set TempBackupRoamingLowDir=C:\_BR\Roaming
...
set ListLocalInclude=%ListDir%\7z_IncludeLocal.txt
set ListLocalExclude=%ListDir%\7z_ExcludeLocal.txt
set ListRecurseExclude=%ListDir%\7z_R_ExcludeLocal.txt
set ListL_Exclude=%ListDir%\ExcludeLocal.txt
:: 
::  Self Backup
...
if not exist "%TempBackupDir[0]%" mkdir "%TempBackupDir[0]%"
if not exist "%TempBackupDir[1]%" mkdir "%TempBackupDir[1]%"
if not exist "%TempBackupDir[2]%" mkdir "%TempBackupDir[2]%"
@timeout /t 1 >nul
if exist !Dir[%isStep%]!\GetDirSize.vbs goto :Step2
:: 
:CreateVBSScripts
...
@timeout /t 2 >nul
for /f "delims=|" %%L in (!ListL_Exclude!) Do (
...
:StepGTRSource
xcopy %Dir001%\NVIDIA\accounts. %TempBackupDir[0]%\NVIDIA\ /R /Y /O /X
!7za! u -y -r !7zaKeyString! "%TempBackupDir%\Local.7z" "%TempBackupDir[0]%\*" -w"%Temp%" -x@"!ListLocalExclude!" -xr@"!ListRecurseExclude!"
rem <

ROBOCOPY "%TempSourceBackupDir%" "!TempBackupDir[%isStep%]!" %RCParams% /LOG:"%RCLogFile%" /XF %RCLocalExcludeFile% /XD %RCLocalExcludeDir% %RCFileDataFlags% /TBD
:: 
goto :cleaning
...
rem start /b /wait 
::
rem if exist "%TempBackupDir%\Local.7z" if %isStep% == 0 (set isStep=1)
...
if exist "%TempBackupDir[0]%" rmdir /S /Q "%TempBackupDir[0]%\"
if exist "%TempBackupDir[1]%" rmdir /S /Q "%TempBackupDir[1]%\"
if exist "%TempBackupDir[2]%" rmdir /S /Q "%TempBackupDir[2]%\"
