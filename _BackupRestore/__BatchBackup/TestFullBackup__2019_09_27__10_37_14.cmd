...
cd /D !Dir[1]!\
echo %~dp1%~n1
REM Echo WScript.Echo Round(CreateObject("Scripting.FileSystemObject").GetFolder(WScript.Arguments(0)).Size/2^^20,2)>GetDirSize.vbs
REM ::  
REM for /f "usebackq delims=;" %%A in (`dir /b *.*`) do (
	REM For /F "delims=" %%B In ('CScript //Nologo "%~dp1%~n1"\GetDirSize.vbs "%%A"') Do (
		REM Set Bytes=%%B
		...
		REM if %%B geq 1 Echo Size of %%A is !Bytes! MB.
	REM )
REM )
REM for /f "usebackq delims=;" %%A in (`dir /b *.*`) do (
	REM echo -- %%~nxA	--	%%~zA			%%A
