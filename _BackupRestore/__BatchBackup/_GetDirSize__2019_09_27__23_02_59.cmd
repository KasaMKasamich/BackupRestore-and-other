...
set sTarget="%1"
set sTargetName="%~n1"
if not exist %~dp0\Target.txt ( echo "%~dp1%~n1">> %~dp0\Target.txt )
if not exist %~dp0\TargetName.txt ( echo %~n1>> %~dp0\TargetName.txt )

for /f "usebackq tokens=*" %%A In ("%~dp0\Target.txt") Do (
	set sTarget="%%A"
)
...
if exist "%~dp0\.sTargetName" ( Del /F /Q "%~dp0\.sTargetName" 1>nul 2>&1 )
if %isSandbox% == 1 ( goto :Stop )
exit
