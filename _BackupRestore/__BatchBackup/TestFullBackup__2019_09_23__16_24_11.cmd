...
for /d %%A In ("%Dir001%\*") Do (
	rem for /f "delims=|" %%i in (!ListL_Exclude!)
	for /f "usebackq delims=" %%i in (`dir /a:d /s /b "%Dir001%\!ListL_Exclude!"`) Do (
		if not exist "%%i" MKLINK /d /h /j "%%~nxA" "%%A"
	)
	rem if not exist "%%~nxA" MKLINK /d /h /j "%%~nxA" "%%A"
)
@timeout /t 92 >nul
