...
rem  7za
set 7zaKeyString=-ssw -m0=LZMA2:d128m:fb256 -myx9 -mmt=16 -ms=off
set 7zaKeyString2=-ssw -mmt=16 -ms=off
set 7zaKeyString3=-ssw -mmt=16
...
rem >> ===================================
if %isStep% == 0 (
	set EndTimeStamp=!DATE:~-4!_!DATE:~3,2!_!DATE:~0,2!__!digit1!!TIME:~1,1!_!TIME:~3,2!_!TIME:~6,2!
	set /A DURATION=!TimeStamp!-!EndTimeStamp!>>%LogFile%
	...
	if not %isStep% == 0 (
		if %isStep% LEQ 2 (
			...
			For /D %%A In ("!TempRoboCopDir[%isStep%]!\*") Do (
				if "!TempRoboCopDir[%isStep%]!\%%~nxA" == "%%A" (
					!7za! u -y -r0 !7zaKeyString! "!TempOutputDir[%isStep%]!\%%~nxA.7z" "%%A" -x@"!7z_ExcludeList[%isStep%]!" -xr@"!7z_ExcludeRecurseList[%isStep%]!"
				)
			)
			!7za! u -y -r0 !7zaKeyString! "!TempOutputDir[%isStep%]!\Microsoft\Credentials.7z" "!Dir[%isStep%]!\Microsoft\Credentials" '-xr@"!7z_ExcludeRecurseFiles!"'
			...
			!7za! u -y -r0 !7zaKeyString! "!TempOutputDir[2]!\Microsoft\Crypto.7z" "%Dir[2]%\Microsoft\Crypto" '-xr@"!7z_ExcludeRecurseFiles!"'
		...
		if %isStep% GEQ 4 (
			...
			if %isStep% LEQ 6 (
				For /D %%A In ("!Dir[%isStep%]!\*") Do (
					if "!Dir[%isStep%]!\%%~nxA" == "%%A" (
						!7za! u -y -r0 !7zaKeyString! "!TempOutputDir[%isStep%]!\%%~nxA.7z" "%%A" -x@"!7z_ExcludeList[%isStep%]!"
					)
				)
			)
		)
		if %isStep% == 0 (
			@rem LocalLow
			!7za! u -y -r0 !7zaKeyString! "!TempOutputDir[%isStep%]!\!FileN[%isStep%]!.7z" "!Dir[%isStep%]!\*" -w"%Temp%" '-xr@"!7z_ExcludeRecurseFiles!"'
			@rem USERNAME
			cd /D %userprofile%\
