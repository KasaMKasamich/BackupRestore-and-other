...
set BackupsDir=F:\Archive\_BackUps\_BackupsArchive
...
@set SelfBackupDir=%~dp0__BatchBackup
...
if %isStep% LEQ 6 (
	if not %isStep% == 0 (
		if %isStep% LEQ 2 (
			for /f "delims=|" %%L in (!7z_List[%isStep%]!) Do (
				if exist "!Dir[%isStep%]!\%%L" (
					!7za! u -r0 !7zaKeyString2! "!OutputDir[%isStep%]!\%%L.7z" "%%L" -w"%Temp%" '-xr@"!7z_ExcludeRecurseFiles[%isStep%]!"'
				)
			)
			@rem !7za! u -r0 !7zaKeyString2! "!OutputDir[%isStep%]!\Microsoft\Credentials.7z" "!Dir[%isStep%]!\Microsoft\Credentials" '-xr@"!7z_ExcludeRecurseFiles!"'
			@rem !7za! u -r0 !7zaKeyString2! "!OutputDir[2]!\Microsoft\Crypto.7z" "%Dir[2]%\Microsoft\Crypto" '-xr@"!7z_ExcludeRecurseFiles!"'
			!7za! u !7zaKeyString2! "!OutputDir[%isStep%]!\!FileN[%isStep%]!.7z" @"!7z_List2[%isStep%]!"
		)
		@rem Users\Public
		if %isStep% == 3 ( !7za! u !7zaKeyString2! "!OutputDir[%isStep%]!\!FileN[%isStep%]!.7z" -w"%Temp%" @"!7z_List[%isStep%]!" )
		@rem Program Files...
		if %isStep% GEQ 4 (
			@rem Common Files...
			if %isStep% LEQ 5 (
				!7za! u !7zaKeyString2! "!OutputDir[%isStep%]!\Common Files.7z" -w"%Temp%" @"!7z_ListCommonFiles[%isStep%]!"
			)
			if %isStep% == 5 (
				...
				@rem !7za! u -y !7zaKeyString2! "!OutputDir[5]!\Microsoft Visual Studio.7z" "!Dir[%isStep%]!\Microsoft Visual Studio\" -w"%Temp%"
				@rem !7za! u -y !7zaKeyString2! "!OutputDir[5]!\Nox.7z" "!Dir[%isStep%]!\Nox\" -w"%Temp%"
			)
			if %isStep% LEQ 6 (
				for /f "delims=|" %%L in (!7z_List[%isStep%]!) Do (
					if exist "!Dir[%isStep%]!\%%L" (
						!7za! u !7zaKeyString2! "!OutputDir[%isStep%]!\%%L.7z" "%%L" -w"%Temp%"
					)
				)
			)
		)
		...
		if %isStep% == 0 (
			...
			@rem Desktop
			!7za! u -r0 !7zaKeyString2! "%BackupsDir%\%TimeStamp%_%FileName%\Desktop.7z" "I:\Users\New\Desktop\*" -w"%Temp%"
			!7za! d -y "%BackupsDir%\%TimeStamp%_%FileName%\Desktop.7z" desktop.ini *.tmp *.dmp *.cookie *.log -r
			@rem LocalLow
			!7za! u -r0 !7zaKeyString2! "!OutputDir[%isStep%]!\!FileN[%isStep%]!.7z" "!Dir[%isStep%]!\*" -w"%Temp%" '-xr@"!7z_ExcludeRecurseFiles!"'
			@rem USERNAME
			cd /D %userprofile%\
			!7za! u !7zaKeyString2! "%BackupsDir%\%TimeStamp%_%FileName%\Users\%USERNAME%\%USERNAME%.7z" -w"%Temp%" @"!7z_List[%isStep%]!"
		)
	)
)
@timeout /t 1 >nul
