...
set TempBackupLocalDir=C:\_BR\Local
set TempBackupLocalLowDir=C:\_BR\LocalLow
...
if not exist "%TempBackupLocalDir%" mkdir "%TempBackupLocalDir%"
if not exist "%TempBackupLocalLowDir%" mkdir "%TempBackupLocalLowDir%"
@timeout /t 1 >nul
...
:StepGTRSource
xcopy %Dir001%\NVIDIA\accounts. %TempBackupLocalDir%\NVIDIA\ /R /Y /O /X
!7za! u -y -r !7zaKeyString! "%TempBackupDir%\Local.7z" "%TempBackupLocalDir%\*" -w"%Temp%" -x@"!ListLocalExclude!" -xr@"!ListRecurseExclude!"
rem <

ROBOCOPY "%TempSourceBackupDir%" "%TempBackupLocalDir%" %RCParams% /LOG:"%RCLogFile%" /XF %RCLocalExcludeFile% /XD %RCLocalExcludeDir% %RCFileDataFlags% /TBD
:: 
goto :cleaning
...
if exist "%TempBackupLocalDir%" rmdir /S /Q "%TempBackupLocalDir%\"
if exist "%TempBackupLocalLowDir%" rmdir /S /Q "%TempBackupLocalLowDir%\"
