...
rem  RoboCopy
set RCParams=/S /ZB /R:3 /W:2 /SL /MT:16 /XA:sh
...
rem  7za
set 7zaKeyString=-ssw -m0=LZMA2:d128m:fb256 -myx9 -slp -mmt=16 -ms=off
...
set tempStep=0
if %DM% == 1 (
	if %CleanTemp% GEQ 1 if %CleanTemp% NEQ 4 ( if exist %TempSourceBackupDir% ( powershell -Command "Get-ChildItem -Path "%TempSourceBackupDir%\*" -Directory  | foreach { $_.Delete()}" ))
	if %CleanTemp% == 2 ( if exist %TempRoboCopDir[0]% ( powershell -Command "Get-ChildItem -Path "%TempRoboCopDir[0]%\*" -Directory  | foreach { $_.Delete()}" ))
	if %CleanTemp% == 3 ( if exist %TempBackupDir%\Users\ ( powershell -Command "Get-ChildItem -Path "%TempBackupDir%\Users\*" -Directory  | foreach { $_.Delete()}" ))
	@rem set %isStep%=1
	@timeout /t 3 >nul
	if %CleanTemp% == 4 (
...
For /d %%A In ("%TempBackupDir%\Users\*") Do (
	for /f "usebackq delims=;" %%i in (`dir /a:a /s /b "%%A"`) do (
		set TimeStamp=%TimeStamp%
		!7za! d -y "%%i" desktop.ini *.tmp *.dmp *.cookie *.log -r >>%LogsDir%\%TimeStamp%_%%i.log
	)
)
exit /b
@timeout /t 360 >nul
