...
if %isStep% GEQ 2 (
	For /D %%L In ("%TempSourceBackupDir%\*") Do (
...
cd /D %TempSourceBackupDir%\
if %isStep% GEQ 2 (
	for /f "delims=|" %%L in (%TempSourceBackupDir%\*) Do (
		for /f "usebackq delims=" %%i in (`dir /a:d /b "%TempSourceBackupDir%\%%L"`) Do (
			if exist "%TempSourceBackupDir%\%%i" rmdir /q "%TempSourceBackupDir%\%%~nxi"
		)
	)
...
if %isStep% LEQ 2 (
	...
	cd /D !Dir[%isStep%]!\
	if not %isStep% == 0 (
		ROBOCOPY "%TempSourceBackupDir%" "!TempAppData[%isStep%]!" %RCParams% /LOG:"!RCLogFile!" /XF !RC_ExcludeRecurseFileList[1]! /XD !RC_ExcludeDirList[%isStep%]! %RCFileDataFlags%
