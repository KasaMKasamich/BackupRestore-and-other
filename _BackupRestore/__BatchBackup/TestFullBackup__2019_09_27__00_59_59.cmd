...
@timeout /t 2 >nul
if not %isStep% == 0 (
	for /f "delims=|" %%L in (!List_Exclude[1]!) Do (
		for /f "usebackq delims=" %%B in (`dir /a:d /b "%TempSourceBackupDir%\%%L"`) Do (
			if "%TempSourceBackupDir%\%%~nxB" == "%%B" (
				echo -- Y -- %TempSourceBackupDir%\%%~nxB	--			%%B
				@rem if exist "%TempSourceBackupDir%\%%B" rmdir /q "%TempSourceBackupDir%\%%~nxB"
			)
		)
	)
)
...
echo rem =============================================================================================================================
For /D %%A In ("!Dir[2]!\*") Do (
	if "%Dir[2]%\%%~nxA" == "%%A" ( echo -- Y -- %Dir[2]%\%%~nxA				%%A 
	) else ( echo -- N -- %Dir[2]%\%%~nxA				%%A )
)
