...
rem >> ===================================
echo -- %~dp0 - %~n0  --  %~dp1 - %~n1
@rem ROBOCOPY "%~dp1%~n1" "%BackupsDir%\Users\%USERNAME%\AppData\!FileN[%isStep%]!\%~n1" %RCParams% /XF !RC_ExcludeRecurseFileList! /XD !RC_ExcludeDirList[%isStep%]! %RCFileDataFlags%
rem << ===================================
	goto :stop
)
@timeout /t 3 >nul
	...
	if not %isStep% == 0 (
		for /f "delims=|" %%L in (!7z_List[%isStep%]!) Do (
			if exist "!Dir[%isStep%]!\%%L" (
				ROBOCOPY "%%L" "%BackupsDir%\Users\%USERNAME%\AppData\!FileN[%isStep%]!\%%L" %RCParams% /LOG:"!RCLogFile!" /XF !RC_ExcludeRecurseFileList! /XD !RC_ExcludeDirList[%isStep%]! %RCFileDataFlags%
			)
		)
	)
)
