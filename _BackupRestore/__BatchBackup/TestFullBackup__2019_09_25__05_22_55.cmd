...
set 7z_IncludeList[0]=""
set 7z_IncludeList[1]=%ListDir%\7z_IncludeLocal.txt
set 7z_IncludeList[2]=%ListDir%\7z_IncludeRoaming.txt
...
if not %isStep% == 0 (
	if exist !Dir[%isStep%]!\GetDirSize.vbs (
		...
		echo %Dir[!isStep!]%
		@rem goto :CreateVBSScripts
	)
)
echo ======================================================
if not %isStep% == 0 (
	!7za! u -y -r !7zaKeyString! "!TempAppData[%isStep%]!\!FileN[%isStep%]!.7z" "%TempDirNBP%\!FileN[%isStep%]!\*" '@"%7z_IncludeList[!isStep!]%"'
)
if not %isStep% == 0 ( echo %isStep%
)
...
!7za! u -y -r !7zaKeyString! "!TempAppData[%isStep%]!\Microsoft\Credentials.7z" "%Dir[0]%\Microsoft\Credentials"
!7za! u -y -r !7zaKeyString! "!TempAppData[%isStep%]!\Microsoft\Credentials.7z" "%Dir[2]%\Microsoft\Credentials"
!7za! u -y -r !7zaKeyString! "!TempAppData[%isStep%]!\Microsoft\Crypto.7z" "%Dir[2]%\Microsoft\Crypto"
::  AppData\LocalLow
!7za! u -y -r !7zaKeyString! "%TempAppData[1]%\%FileN[1]%.7z" "%Dir[1]%\*" -w"%Temp%"
::  AppData\Roaming
if not %isStep% == 1 (
	!7za! u -y -r !7zaKeyString! "!TempAppData[%isStep%]!\!FileN[%isStep%]!.7z" "%TempDirNBP%\!FileN[%isStep%]!\*"
)
...
:: 0
@rem Microsoft\Windows\Themes\ %TempDirNBP%\
