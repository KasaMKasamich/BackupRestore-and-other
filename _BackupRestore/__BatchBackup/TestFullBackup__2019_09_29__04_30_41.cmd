...
set TempRoboCopDir[3]=C:\_BR\_RoboTemp\Program Files
set TempRoboCopDir[4]=C:\_BR\_RoboTemp\Program Files (x86)
set TempRoboCopDir[5]=C:\_BR\_RoboTemp\ProgramData
...
REM for /f "usebackq delims=;" %%A in (`dir /s /b *.*`) do (
	REM For /F "delims=" %%B In ('CScript //Nologo "GetDirSize.vbs "%%A"') Do (
		REM if "%%~nxA" == "%%A" (
			REM if %%B LSS 1 del /f /s /q "%%A" >> %LogFile%
			REM if %%B LSS 1 Echo Size of %%A is %%B bytes. >> %LogFile%
		REM )
	REM )
REM )
REM for /f "usebackq delims=;" %%A in (`dir /s /b *.*`) do (
	REM if "%%~nxA" == "%%A" (
		REM If %%~zA LSS 1 del /f /s /q "%%A"
	REM )
REM )
REM for /f "usebackq delims=;" %%A in (`dir /a:h-s /s /b *.*`) do (
	REM if "%%~nxA" == "%%A" (
		REM If %%~zA LSS 1 del /f /s /q /a:h-s "%%A"
	REM )
REM )
...
@timeout /t 1 >nul
if exist !Dir[1]!\GetDirSize.vbs ( set /a isExistVBS=!isExistVBS!+1 )
if exist !Dir[2]!\GetDirSize.vbs ( set /a isExistVBS=!isExistVBS!+1 )
if exist %TempSourceBackupDir%\GetDirSize.vbs ( set /a isExistVBS=!isExistVBS!+1 )
if exist !TempRoboCopDir[1]!\GetDirSize.vbs ( set /a isExistVBS=!isExistVBS!+1 )
if exist !TempRoboCopDir[2]!\GetDirSize.vbs ( set /a isExistVBS=!isExistVBS!+1 )
echo -- isExistVBS = %isExistVBS%
if %isStep% LEQ 2 (
...
Echo WScript.Echo CreateObject("Scripting.FileSystemObject").GetFolder(WScript.Arguments(0)).Size>GetDirSize.vbs
cd /d !TempRoboCopDir[1]!\
Echo WScript.Echo CreateObject("Scripting.FileSystemObject").GetFolder(WScript.Arguments(0)).Size>GetDirSize.vbs
cd /d !TempRoboCopDir[2]!\
Echo WScript.Echo CreateObject("Scripting.FileSystemObject").GetFolder(WScript.Arguments(0)).Size>GetDirSize.vbs
cd /D !TempOutputDir[%isStep%]!\
Echo WScript.Echo CreateObject("Scripting.FileSystemObject").GetFolder(WScript.Arguments(0)).Size>GetDirSize.vbs
@timeout /t 1 >nul
rem 
:RoboCopy
...
if %isStep% LEQ 6 (
	@rem if not exist "!TempOutputDir[%isStep%]!\!FileN[%isStep%]!.7z" ( call :NotBatchAppDataPacking )
	cd /D !Dir[%isStep%]!\
	if not %isStep% == 0 (
		if %isStep% LEQ 2 (
	...
	) else if %isStep% GEQ 4 if %isStep% LEQ 6 (
		@rem Users\Public & Program Files...
		cd /D !Dir[%isStep%]!\
		!7za! u -y !7zaKeyString! "!TempOutputDir[%isStep%]!\!FileN[%isStep%]!.7z" @"!7z_List[%isStep%]!"
	) else if %isStep% == 0 (
		@rem LocalLow
...
REM set Dir[3]=C:\Users\Public
REM set Dir[4]=%ProgramFiles%
REM set Dir[5]=C:\Program Files (x86)
REM set Dir[6]=C:\ProgramData

REM set TempOutputDir[0]=%TempBackupDir%\Users\%USERNAME%\AppData\LocalLow
REM set TempOutputDir[1]=%TempBackupDir%\Users\%USERNAME%\AppData\Local
REM set TempOutputDir[2]=%TempBackupDir%\Users\%USERNAME%\AppData\Roaming
REM set TempOutputDir[3]=%TempBackupDir%\%UsersDir[0]%
REM set TempOutputDir[4]=C:\_BR\Program Files
REM set TempOutputDir[5]=C:\_BR\Program Files (x86)
REM set TempOutputDir[6]=C:\_BR\ProgramData

REM set FileN[0]=LocalLow
REM set FileN[1]=Local
REM set FileN[2]=Roaming
REM set FileN[3]=Public
REM set FileN[4]=ProgramFiles
REM set FileN[5]=ProgramFiles_x86
REM set FileN[6]=ProgramData

REM set 7z_List[0]=%ListDir%\7z_UserProfile.txt
REM set 7z_List[1]=%ListDir%\7z_Local.txt
REM set 7z_List[2]=%ListDir%\7z_Roaming.txt
REM set 7z_List[3]=%ListDir%\7z_Public.txt
REM set 7z_List[4]=%ListDir%\7z_ProgramFiles.txt
REM set 7z_List[5]=%ListDir%\7z_ProgramFiles_x86.txt
REM set 7z_List[6]=%ListDir%\7z_ProgramData.txt
...
@rem if not exist "!TempOutputDir[0]!\LocalLow.7z" if %isStep% == 1 (set isStep=2)
@rem if not exist "!TempOutputDir[1]!\Local.7z" if %isStep% == 1 (set isStep=1)
@rem if not exist "!TempOutputDir[2]!\Roaming.7z" if %isStep% == 2 (set isStep=2)
rem if exist "%TempBackupDir%\Roaming.7z" if %isStep% == 2 (set isStep=3)
@rem if %isStep% == 1 ( goto :Pack )
