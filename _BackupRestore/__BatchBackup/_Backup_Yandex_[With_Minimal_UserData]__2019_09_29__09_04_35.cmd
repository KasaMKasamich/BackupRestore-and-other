...
set AppDataDir[2]=%userprofile%\AppData\LocalLow\Yandex\Updater
set AppDataDir[3]=%userprofile%\AppData\Local\Yandex\YandexBrowser\Application
...
!7za! u -y !7zaKeyString! "%TempBackupDir%\TempYandex\Updater.7z" "%AppDataDir[2]%" @"!7z_List[1]!" -w"%Temp%" >> %LogFile%
cd /D %userprofile%\AppData\Local\
!7za! u -y !7zaKeyString! "%TempBackupDir%\TempYandex\Application.7z" "%AppDataDir[3]%" @"!7z_List[1]!" -w"%Temp%" >> %LogFile%
cd /D "C:\Program Files (x86)\"
!7za! u -y !7zaKeyString! "%TempBackupDir%\TempYandex\Service_Update.7z" "%AppDataDir[4]%" @"!7z_List[1]!" -w"%Temp%" >> %LogFile%

cd /D %CustomUserDataDir%
!7za! u -y !7zaKeyString! "%TempBackupDir%\TempYandex\%FileName[0]%.7z" @"!7z_List[0]!" -w"%Temp%" >> %LogFile%

cd /D %TempBackupDir%\TempYandex\
!7za! u -y !7zaKeyString! "%TempBackupDir%\%FileName[1]%.7z" *  @"!7z_List[1]!" -w"%Temp%" >> %LogFile%

cd /D %TempBackupDir%
@rem Rename & Move UserData
move /y "%TempBackupDir%\TempYandex\%FileName[0]%.7z" %TempBackupDir% >> %LogFile%
ren "%TempBackupDir%\%FileName[0]%.7z" "%TimeStamp%__%FileName[0]%.7z" >> %LogFile%
move /y "%TimeStamp%__%FileName[0]%.7z" %BackupsDir% >> %LogFile%

@rem Move YandexBrowser
move /y "TempYandex\"%TempBackupDir%\%FileName[1]%.7z" %BackupsDir% >> %LogFile%
