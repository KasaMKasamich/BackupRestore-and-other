...
call :Test !TimeStamp! !RCLogFile!
:Test
for /f "delims=|" %%L in (!ListDir!\RC_Roaming.txt) Do (
	rem xcopy %Dir[2]%\%%L %TempDirNBP%\%FileN[2]%2\ /R /Y /O /X
	ROBOCOPY "%Dir[2]%\%%L" "%TempDirNBP%\%FileN[2]%2\" %RCParams2% /LOG:"!RCLogFile!" %RCFileDataFlags%
	for /f "usebackq delims=" %%i in (`dir /a:d /b "%Dir[2]%\%%L"`) Do (
		if exist "%Dir[2]%\%%i" (
			echo ====================================================
			echo %Dir[2]%\%%L
			echo ====================================================
			rem xcopy %Dir[2]%\%%L\%%i\ %TempDirNBP%\%FileN[2]%2\%%L%%i /R /Y /O /X
			rem ROBOCOPY "%Dir[2]%\%%L" "%TempDirNBP%\%FileN[2]%2\%%L\%%~nxi" %RCParams2% /LOG:"!RCLogFile!" %RCFileDataFlags%
		)
	)
)
...
!7za! u -y -r !7zaKeyString! "%TempDirNBP%\%FileN[0]%\Microsoft\Credentials.7z" "%Dir[0]%\Microsoft\Credentials"
!7za! u -y -r !7zaKeyString! "%TempDirNBP%\%FileN[2]%\Microsoft\Credentials.7z" "%Dir[2]%\Microsoft\Credentials"
!7za! u -y -r !7zaKeyString! "%TempDirNBP%\%FileN[2]%\Microsoft\Crypto.7z" "%Dir[2]%\Microsoft\Crypto"
