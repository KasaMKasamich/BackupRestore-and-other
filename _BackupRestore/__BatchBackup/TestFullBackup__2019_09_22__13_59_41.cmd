...
set RCLocalExcludeDir="IceDragon" "Logs" "Recent" "Report*" "Temporary*" "*Crash*" "*Temp*" "*History*" "*Log*" "Application*" "Microsoft" "Diagnostics" "Packages" "Comms" "NVIDIA*" "Programs" "Publishers" "TempCheckUpdate" "fontconfig" "{*}" "*Folder" "e2eSoft" "speech" "TechSmith" "*Tile*" "Wondershare" "VirtualStore" "ConnectedDevicesPlatform"
...
xcopy %Dir001%\NVIDIA\accounts. %TempBackupDir%\NVIDIA\ /R /Y /Z
...
xcopy %Dir001%\NVIDIA\accounts. %TempBackupDir%\NVIDIA\ /S /R /Y /Z /K /E
rem ROBOCOPY %Dir001%\NVIDIA\accounts. %TempBackupDir%\NVIDIA\accounts. %RCParams% /LOG:"%RCLogFile%" %RCFileDataFlags%
...
rem start /b /wait 
!7za! u -y -r !7zaKeyString! "%TempBackupDir%\Local.7z" "%TempSourceBackupDir%\*" -w"%Temp%" -x@"%ListLocalExclude%" -xr@"%ListRecurseExclude%"
