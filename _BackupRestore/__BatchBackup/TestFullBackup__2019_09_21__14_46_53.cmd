...
rem start /b /wait 
F:\_BackupRestore\7za\7za.exe a -ssw -mx9 "%TempBackupDir%\Viber.7z" "%Dir001%\Viber" -x@"%ListLocalExclude%" -xr@"%ListRecurseExclude%"
F:\_BackupRestore\7za\7za.exe a -ssw -mx9 -r- "%TempBackupDir%\Viber2.7z" "%Dir001%\Viber" -x@"%ListLocalExclude%" -xr@"%ListRecurseExclude%"
F:\_BackupRestore\7z\1900\x64\7za.exe u -ssw -r- -m0=LZMA2:d128m:fb256 -myx9 -slp -mmt=32 -ms=off "%TempBackupDir%\Viber3.7z" "%Dir001%\Viber" -x@"%ListLocalExclude%" -xr@"%ListRecurseExclude%"
F:\_BackupRestore\7z\1900\x64\7za.exe a -ssw -r- -m0=LZMA2:yx9:d128m:fb256 -slp -mmt=32 -ms=off "%TempBackupDir%\Viber4.7z" "%Dir001%\Viber" -x@"%ListLocalExclude%" -xr@"%ListRecurseExclude%"
F:\_BackupRestore\7z\1900\x64\7za.exe a -ssw -r- -m0=LZMA2:x9:yx9:d128m:fb256 -slp -mmt=32 -ms=off "%TempBackupDir%\Viber5.7z" "%Dir001%\Viber" -x@"%ListLocalExclude%" -xr@"%ListRecurseExclude%"
if errorlevel 255 echo user_stopped_the_process rem goto:user_stopped_the_process
if errorlevel 8 echo not_enough_memory rem goto:not_enough_memory
if errorlevel 7 echo command_line_error rem goto:command_line_error
if errorlevel 2 echo fatal_error rem goto:fatal_error
if errorlevel 1 echo ok_warnings rem goto:ok_warnings
:: 
pause
...
For /D %%A In ("%TempSourceBackupDir%\*") Do (
	For /F "delims=" %%B In ('CScript //Nologo %TempSourceBackupDir%\GetDirSize.vbs "%%A"') Do (
		if %%B geq !MB! start /b /wait F:\_BackupRestore\7z\1900\x64\7za.exe u -t7z -r0 -ssw -mx9 -slp -scsWIN -mmt=4 -ms=off -y "%TempBackupDir%\%%~nxA.7z" "%%A" -w"%TempBackupDir%" -x@"%ListLocalExclude%" -xr@"%ListRecurseExclude%"
	)
