...
if %isStep% == 0 (
cd /D !Dir[4]!\
					for /f "usebackq delims=" %%B in (`dir /a:d /b "!7z_List[4]!"`) Do (
						if "%%~nxB" == "%%B" (
							echo -- %%~nxB  --  %%B
						)
					)
@rem				for /f "delims=|" %%L in (!7z_List[4]!) Do (
@rem					for /f "usebackq delims=" %%B in (`dir /a:d /b "!7z_List[4]!"`) Do (
@rem						if "%%~nxB" == "%%B" (
@rem							echo -- %%~nxB  --  %%B
@rem							!7za! u !7zaKeyString2! "!TempOutputDir[%isStep%]!\%%~nxB.7z" "%%B" -w"%Temp%"
@rem						)
@rem					)
@rem				)
...
@timeout /t 3 >nul
		@rem Users\Public
		if %isStep% == 3 (
			!7za! u !7zaKeyString2! "!TempOutputDir[%isStep%]!\!FileN[%isStep%]!.7z" @"!7z_List[%isStep%]!"
		)
		@rem Program Files...
		if %isStep% GEQ 4 (
			@rem Program Files...
			...
			if %isStep% LEQ 6 (
@rem	for /f "delims=|" %%L in (!List_Exclude[%isStep%]!) Do (
@rem				For /D %%A In ("!Dir[%isStep%]!\*") Do (
				for /f "delims=|" %%L in (!7z_List[%isStep%]!) Do (
					for /f "usebackq delims=" %%B in (`dir /a:d /b "!Dir[%isStep%]!\%%L"`) Do (
						if "%%~nxB" == "%%B" (
							echo -- %%~nxB  --  %%B
							!7za! u !7zaKeyString2! "!TempOutputDir[%isStep%]!\%%~nxB.7z" "%%B" -w"%Temp%"
						)
					)
				)
@rem 				For /D %%A In ("!7z_List[%isStep%]!") Do (
@rem 					!7za! u !7zaKeyString2! "!TempOutputDir[%isStep%]!\%%~nxA.7z" "%%A" -w"%Temp%"
@rem 					-x@"!7z_ExcludeList[%isStep%]!"
					rem if "!Dir[%isStep%]!\%%~nxA" == "%%A" (
					rem )
@rem 				)
			)
		)
...
if %isStep% LEQ 6 (
	...
	if not %isStep% == 0 (
		if %isStep% LEQ 2 (
			...
			For /D %%A In ("!TempRoboCopDir[%isStep%]!\*") Do (
				if "!TempRoboCopDir[%isStep%]!\%%~nxA" == "%%A" (
					!7za! u -r0 !7zaKeyString! "!TempOutputDir[%isStep%]!\%%~nxA.7z" "%%A" -x@"!7z_ExcludeList[%isStep%]!" -xr@"!7z_ExcludeRecurseList[%isStep%]!"
				)
			)
			!7za! u -r0 !7zaKeyString! "!TempOutputDir[%isStep%]!\Microsoft\Credentials.7z" "!Dir[%isStep%]!\Microsoft\Credentials" '-xr@"!7z_ExcludeRecurseFiles!"'
			...
			!7za! u -r0 !7zaKeyString! "!TempOutputDir[2]!\Microsoft\Crypto.7z" "%Dir[2]%\Microsoft\Crypto" '-xr@"!7z_ExcludeRecurseFiles!"'
			!7za! u !7zaKeyString! "!TempOutputDir[%isStep%]!\!FileN[%isStep%]!.7z" @"!7z_List[%isStep%]!"
		)
	)
	else (
		cd /D !Dir[%isStep%]!\
		@rem Users\Public
		if %isStep% == 3 ( !7za! u !7zaKeyString2! "!TempOutputDir[%isStep%]!\!FileN[%isStep%]!.7z" @"!7z_List[%isStep%]!" )
		@rem Program Files...
		if %isStep% GEQ 4 (
			@rem Program Files...
			if %isStep% LEQ 5 (
			...
			if %isStep% LEQ 6 (
				For /D %%A In ("!Dir[%isStep%]!\*") Do (
					!7za! u -r0 !7zaKeyString2! "!TempOutputDir[%isStep%]!\%%~nxA.7z" "%%A" -w"%Temp%" -x@"!7z_ExcludeList[%isStep%]!"
					rem if "!Dir[%isStep%]!\%%~nxA" == "%%A" (
					rem )
				)
			)
		...
		if %isStep% == 0 (
			@rem LocalLow
			!7za! u -r0 !7zaKeyString! "!TempOutputDir[%isStep%]!\!FileN[%isStep%]!.7z" "!Dir[%isStep%]!\*" -w"%Temp%" '-xr@"!7z_ExcludeRecurseFiles!"'
			@rem USERNAME
			cd /D %userprofile%\
			!7za! u !7zaKeyString! "%TempBackupDir%\Users\%USERNAME%\%USERNAME%.7z" @"!7z_List[%isStep%]!"
		)
	)
)
@timeout /t 1 >nul
