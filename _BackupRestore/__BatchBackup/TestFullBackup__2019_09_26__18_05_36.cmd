...
if %DM% == 1 (
	...
	if %cleantemp% == 2 ( if exist %TempSourceBackupDir% ( powershell -Command "Get-ChildItem -Path "%TempRoboCopDir[0]%\*" -Directory  | foreach { $_.Delete()}" ))
	if %cleantemp% == 3 ( if exist %TempSourceBackupDir% ( powershell -Command "Get-ChildItem -Path "%TempBackupDir%\Users\*" -Directory  | foreach { $_.Delete()}" ))
	set isStep=1
	@timeout /t 3 >nul
)
...
For /d %%A In ("%TempBackupDir%\Users\*") Do (
	for /f "usebackq delims=;" %%i in (`dir /a:a /s /b "%%A"`) do (
		!7za! d -y "%%i" desktop.ini *.tmp *.dmp *.cookie *.log -r
	)
)
exit /b
@timeout /t 360 >nul
